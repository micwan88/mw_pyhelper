...
def test_postmsg_without_proxy(mock_post: MagicMock):
...
@patch('mw_pyhelper.messaging.telegrambot.requests.post')
def test_postmsg_with_proxy(mock_post: MagicMock):
    mock_post.return_value = _ok_response()
    ...
    assert result == 0
    assert mock_post.call_args.kwargs['proxies'] == PROXY


@patch('mw_pyhelper.messaging.telegrambot.requests.post')
def test_postmsg_with_botname_prefixes_and_escapes(mock_post: MagicMock):
    mock_post.return_value = _ok_response()
    ...
    assert sent_text == 'MyBot:a &lt; b &amp; c'


@patch('mw_pyhelper.messaging.telegrambot.requests.post')
def test_postmsg_http_error_returns_minus_one(mock_post: MagicMock):
    response = MagicMock()
    ...
    assert bot.postmsg(CHAT_IDS, 'hello') == -1
